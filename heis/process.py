...
def numerical_columns(df, column_name, column_property):
    ...
    if len(elements) == 2:
        ...
        if elements[2] == "+":
            df[column_name] = df[elements[1]].fillna(
                0) * float(elements[0]) + df[elements[4]].fillna(0) * int(elements[3])
...
def make_table_standard(df, table_name, year):
    ...
    for column_name in properties['columns'].keys():
        column_property = utils.get_version(
            properties['columns'][column_name], year)
...
def get_column_order(df, table_name):
    ...
    new_order = [column for column in properties['order']
...
def load_table(table_name, from_year=None, to_year=None, standardize=False, note_year=None, parquets_directory=Defaults.parquets_dir):
    ...
    for year in range(start, end):
        table = pd.read_parquet(parquets_directory.joinpath(
            f"{year}_{table_name}.parquet"))
        if standardize:
            table = make_table_standard(
                df=table, table_name=table_name, year=year)
        if note_year:
            table["Year"] = year
...
def get_part_names(id_column: pd.Series, part_name: str, year: int):
    part_codes = get_part_code(
        id_column=id_column, part_name=part_name, year=year)
...
def get_commodity_data(commodity_name: str, year: int, note_duration=False):
    property_list = _get_commodity_properties(
        commodity_name=commodity_name, year=year)
    table_collection = []
    for table_name, code_intervals, duration in property_list:
        table = load_table(table_name=table_name,
                           from_year=year, standardize=True)
        filt = _build_filter_from_intervals(
            df=table, code_intervals=code_intervals)
