...
def download_a_file(year:int, replace:bool=True):
    ...
    local_path = defaults.compressed_dir.joinpath(file_name)
...
def download_files(from_year:int|None=None, to_year:int|None=None, replace:bool=False):
    ...
    pathlib.Path(defaults.compressed_dir).mkdir(exist_ok =True, parents=True)
...
def extract_a_file(year):
    file_path = defaults.compressed_dir.joinpath(f"{year}.rar")
